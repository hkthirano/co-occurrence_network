...
def show(df, auc_type, network, interact, smax, kave, nn, dir_path):
    ...
    for method in method_list:
        x = d_use.loc[d_use['calc-method'] == method, 'sampling'].unique()
    ...
    plt.title('network:{} | interact:{} \nsmax:{} | kave:{} | nn:{} | \nsampling:{}'.format(network, interact, smax, kave, nn, x))
...
for network in network_list:
    for interact in interact_list:
        ...
        # 保存用ディレクトリ作成
        dir_path = 'img/show-method-sampling/st-{}/in-{}'.format(network, interact)
        ...
        for smax in smax_list:
            for kave in kave_list:
                for nn in nn_list:
                    ...
                    #show(df, 'roc', network, interact, smax, kave, nn, dir_path)

                    show(df, 'prc', network, interact, smax, kave, nn, dir_path)