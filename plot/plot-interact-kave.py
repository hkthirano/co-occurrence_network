...
def show(df, auc_type, network, method, smax, nn, sampling, dir_path):
    ...
    for interact in interact_list:
        x = d_use.loc[d_use['interact'] == interact, 'k-ave'].unique()
    ...
    plt.title('method:{} \nnetwork:{} | smax:{} | nn:{} | sampling:{} | \nkave:{}'.format(method, network, smax, nn, sampling, x))
...
for network in network_list:
    for method in method_list:
        ...
        # 保存用ディレクトリ作成
        dir_path = 'img/show-interact-kave/st-{}/{}'.format(network, method)
        ...
        for smax in smax_list:
            for nn in nn_list:
                for sampling in sampling_list:
                    ...
                    #show(df, 'roc', network, method, smax, nn, sampling, dir_path)

                    show(df, 'prc', network, method, smax, nn, sampling, dir_path)